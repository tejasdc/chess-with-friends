...
- `GameDO.init` parses and validates its complete request before the actor's storage-only compare-or-create sequence. It is idempotent **by value**: an identical immutable game ID, players, handles, and time control succeeds; any mismatch fails loudly and cannot replace the first stored value.
...
- `canonicalize` snapshots the persisted handoff generation, queries D1, revalidates that generation, and retries if it changed before setting the next wake. The chosen wake is the minimum of: the next accepted occurrence, a pending schedule's `startAt + 60 seconds` expiry deadline, and the retry time of claimed unfinished effects.
